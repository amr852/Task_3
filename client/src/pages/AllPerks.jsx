...
export default function AllPerks() {
  ...
  // Stable loader (depends on current search/filter)
  const loadAllPerks = useCallback(async () => {
    ...
    try {
      ...
      setPerks(Array.isArray(res?.data?.perks) ? res.data.perks : [])
    } catch (err) {
  ...
  // Initial fetch (instant)
  useEffect(() => {
    loadAllPerks()
  }, [loadAllPerks])

  // Auto-search with debounce; skip the very first render to avoid double call
  ...
  // Build unique merchant list whenever perks change
  useEffect(() => {
    const merchants = perks
      .map(p => p?.merchant)
      .filter(m => typeof m === 'string' && m.trim())
    setUniqueMerchants([...new Set(merchants)].sort((a, b) => a.localeCompare(b)))
  }, [perks])

  // Handlers
  function handleSearch(e) {
    e.preventDefault()
    loadAllPerks() // immediate fetch (bypasses debounce)
  }
    ...
    <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                aria-label="Search perks by name"
                value={searchQuery}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {perks.map((perk) => (
          <Link
            key={perk._id}
            to={`/perks/${perk._id}`} // <-- make sure this route exists
          ...
          >
            ...
            <div className="text-sm text-zinc-600 space-y-1">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="capitalize">{perk?.category || 'uncategorized'}</span>
              </div>
              {Number(perk?.discountPercent) > 0 && (
            ...
            {perk?.createdBy && (
              <div className="mt-3 pt-3 border-t border-zinc-200 text-xs text-zinc-500">
                Created by: {perk.createdBy.name || perk.createdBy.email || 'Unknown'}
              </div>
            )}
          </Link>
        ))}
